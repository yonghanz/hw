...
for method in methods:
    ...
    plt.plot(t, results[method]['x'], label=f"{method} - Displacement", color='blue')
    plt.xlabel('Time (s)')
    plt.ylabel('Displacement x (in)')
    plt.title(f'Displacement x(t) - {method}')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{method}_displacement.png", dpi=300)  # 儲存圖表
    plt.close()

    # 速度 v(t)
    plt.figure(figsize=(10, 4))
    plt.plot(t, results[method]['v'], label=f"{method} - Velocity", color='green')
    plt.xlabel('Time (s)')
    plt.ylabel('Velocity v (in/s)')
    plt.title(f'Velocity v(t) - {method}')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{method}_velocity.png", dpi=300)  # 儲存圖表
    plt.close()

    # 加速度 a(t)
    plt.figure(figsize=(10, 4))
    plt.plot(t, results[method]['a'], label=f"{method} - Acceleration", color='red')
    plt.xlabel('Time (s)')
    plt.ylabel('Acceleration a (in/s²)')
    plt.title(f'Acceleration a(t) - {method}')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{method}_acceleration.png", dpi=300)  # 儲存圖表
    plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("effective_force.png", dpi=300)  # 儲存圖表
plt.close()